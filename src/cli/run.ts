...
export async function runCommand(options: RunOptions): Promise<void> {
  ...
  let meta: Awaited<ReturnType<typeof fetchPRMeta>>;
  let diffs: Awaited<ReturnType<typeof fetchDiff>>;
  try {
  ...
  try {
    if (orgApiKey) {
      ...
      findings = await runReview(config, diffs, (status) => {
        spinner.text = status;
      });
    }
  } catch (error: any) {
  ...
  let generatedAgentPrompt: string | undefined;
  if (options.prompt || options.promptOutput || options.promptComment) {
    const { generateAgentPrompt } = await import('../output/agentPrompt.js');

    // Fetch the raw diff patches for context
    generatedAgentPrompt = generateAgentPrompt({
      findings,
      diffs,
      prUrl:   options.pr,
      prTitle: meta.title,
    ...
    if (options.promptOutput) {
      ...
      fs.writeFileSync(options.promptOutput, generatedAgentPrompt, 'utf8');
      ...
      console.log(generatedAgentPrompt);
      console.log('\n' + chalk.cyan('─'.repeat(60)) + '\n');
    }
  }
  ...
  // 10b. Post prompt comment if requested
  if (options.promptComment && !options.dryRun && generatedAgentPrompt && findings.length > 0) {
    try {
      const promptBody = `### 🤖 GemReview AI Agent Fix Prompt\n\n<details>\n<summary>Click to view complete prompt instructions to automatically resolve these findings</summary>\n\n\`\`\`markdown\n${generatedAgentPrompt}\n\`\`\`\n\n</details>\n\n---\n*GemReview v1.0.0*`;
      ...
      await logger.debug('Agent fix prompt comment posted to PR');
    } catch (error: any) {
      await logger.debug(`Failed to post prompt comment: ${error.message}`);
    }
  }
  ...
  // 13. Fail CI check if severity threshold was reached
  if (shouldFailCI) {
    await logger.error(
