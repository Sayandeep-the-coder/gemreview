...
export async function configShowCommand(): Promise<void> {
  ...
  try {
    ...
    if (typeof config.gemini_api_key === 'string' && config.gemini_api_key.trim() !== '') {
      try {
        const { fetchCurrentModel, formatTokens } = await import('../gemini/models.js');
        const liveModel = await fetchCurrentModel(config.gemini_api_key, config.model);
        console.log(chalk.bold('  🤖 Current Model (Live from Google AI Studio):'));
        console.log(`     Model ID:    ${chalk.cyan.bold(liveModel.id)} [${liveModel.quota.family}]`);
        console.log(`     Context:     ${chalk.white(formatTokens(liveModel.inputTokenLimit))} tokens`);
        console.log(`     Max Output:  ${chalk.white(formatTokens(liveModel.outputTokenLimit))} tokens`);
        console.log(`     Free Quota:  ${chalk.yellow(`${liveModel.quota.rpm} | ${liveModel.quota.tpm} | ${liveModel.quota.rpd}`)}\n`);
      } catch {
        // Continue quietly if offline
      }
    } else {
      console.log('');
    }
