...
program
  ...
  .option('--prompt-comment', 'Post the AI agent fix prompt as a PR comment', false)
  .action(async (opts) => {
    await runCommand({
      pr: opts.pr,
      dryRun: opts.dryRun,
      ...
      prompt: opts.prompt || opts.promptOutput !== undefined || Boolean(opts.promptComment),
      promptOutput: opts.promptOutput,
      promptComment: opts.promptComment,
