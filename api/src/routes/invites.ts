...
inviteRoutes.post('/:token/accept', authMiddleware, async (c) => {
  ...
  // For GitHub username invites, verify the user's githubLogin matches
  if (invite.type === 'github' && invite.githubLogin) {
    if (!user.githubLogin || user.githubLogin.toLowerCase() !== invite.githubLogin.toLowerCase()) {
      return c.json({ error: 'This invite was sent to a different GitHub user' }, 403);
    }
  }
